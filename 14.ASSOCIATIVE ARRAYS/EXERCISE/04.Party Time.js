function solve(input){
    ...
    function createOutput(guests) {
        // let output = [];
        // guests.vip.forEach(vip => {
        //     output.push(vip);
        // })
        // guests.regular.forEach(regular => {
        //     output.push(regular);
        // })
    ...
    let partiIndex = input.indexOf('PARTY');
    let guestList = createList(input.slice(0, partiIndex));
    let removedList = removeGuests(input.splice(partiIndex + 1), guestList);
